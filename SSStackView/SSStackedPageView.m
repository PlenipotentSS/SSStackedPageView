...
- (void)layoutSubviews
{
    ...
    self.theScrollView.frame = CGRectMake(0.f, OFFSET_TOP, CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds));
    self.theScrollView.contentSize = CGSizeMake(CGRectGetWidth(self.bounds), MAX(CGRectGetHeight(self.bounds), self.pageCount * PAGE_PEAK));
}
...
- (void)reloadVisiblePages
{
    ...
    for (NSInteger i = start; i < stop; i++) {
        ...
        CGFloat yOrigin = CGRectGetMinY(page.frame);
...
- (void)tapped:(UIGestureRecognizer*)sender
{
    ...
    NSLog(@"x: %f y: %f",tappedPoint.x,tappedPoint.y);
}
